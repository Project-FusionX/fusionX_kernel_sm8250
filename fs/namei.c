...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (!IS_ERR(dentry) && dentry->d_inode && unlikely(dentry->d_inode->i_state & INODE_STATE_SUS_PATH) && likely(current_cred()->user->android_kabi_reserved2 & INODE_STATE_SUS_PATH)) {
		dput(dentry);
		return ERR_PTR(-ENOENT);
	}
#endif
